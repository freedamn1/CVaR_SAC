...
class WCSACAgent(Agent):
    ...
    def __init__(
        ...
        risk_eps = max(float(self.risk_level), 1e-6)
        normal = tdist.normal.Normal(torch.tensor([0.0]), torch.tensor([1.0]))
        self.pdf_cdf = (
            normal.log_prob(normal.icdf(torch.tensor(risk_eps))).exp() / risk_eps
    ...
    def update_actor_and_alpha_and_beta(self, obs, action_taken, logger, step):
        ...
        actor_loss = torch.mean(
            ...
            - actor_Q
            + (self.beta.detach() - damp) * (actor_QC + self.pdf_cdf * torch.sqrt(actor_VC))
        )
        ...
        logger.log(
            "train/actor_cost",
            torch.mean(actor_QC + self.pdf_cdf * torch.sqrt(actor_VC)),
            step,
        )
